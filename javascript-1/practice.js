...
// Code Here
const doubled = myNumbers.map(el => {
	return el*2;
})
//////////////////PROBLEM 15////////////////////
...
// Code Here
const filtered = myNumbers.filter(el => {
	if(el > 100){
		return el;
	}
})
...
// Code Here
const total = myNumbers.reduce((acc,el) => {
	return acc + el;
});
//////////////////PROBLEM 17////////////////////
...
myNumbers.forEach((el, ind) => {
	myNumbersIndex.push(ind);
});
...
// Code Here
const enemies = people.filter((el, ind) => {
	for(let key in el){
		//console.log(el.name);
    if(el[key] === false){
      return el;
...
const totallyAwesome = people.reduce((acc,el,ind) => {
	//console.log(el.awesomeLevel);
